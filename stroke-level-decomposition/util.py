...
# 笔画
alphabet_strokelet = '<12345$'
alp2num_strokelet = {}
for index, char in enumerate(alphabet_strokelet):
    alp2num_strokelet[char] = index
...
if config['alphabet'] == 'ctw':
    lines_temp = open('./data/strokelet27533.txt', 'r', encoding='utf-8').readlines()
    lines = []
    for line in lines_temp:
        if line[0] in ctw_word:
            lines.append(line)
elif config['alphabet'] == 'korean+3755':
    lines = open('./data/strokelet_korean_3755.txt', 'r', encoding='utf-8').readlines()
elif config['alphabet'] == 'korean':
    lines = open('./data/korean_stroke.txt', 'r', encoding='utf-8').readlines()
elif config['alphabet'] == 27533:
    lines = open('./data/strokelet27533.txt', 'r', encoding='utf-8').readlines()
else:
    lines = open('./data/strokelet.txt','r',encoding='utf-8').readlines()
for line in lines:
    try:
        if config['alphabet'] == 'korean':
            line = line.replace(' ','')
            word = line[0]
            storkelist = line[1:]
        elif config['alphabet'] == 'korean+3755':
            ...
            word = line[0]
            storkelist = line[1:]
            # word, storkelist = line.split()
        else:
            word, _, storkelist = line.split()
        # 限定候选词在1000个里面
        if config['constrain']:
            if word not in alphabet_character[alphabet_character.index('途'):alphabet_character.index('$')]:
                continue
    except:
        print(line)
        exit(0)
...
confusing_strokelet_dic = {}
for i in character_to_strokelist.keys():
    value = character_to_strokelist[i]
    if value not in confusing_strokelet_dic:
        confusing_strokelet_dic[value] = [i]
    else:
        confusing_strokelet_dic[value].append(i)
...
def get_data_package():
    ...
    # test_dataloader, test_dataset = get_dataloader(config['test_dataset'])
...
def converter(mode, label):
    ...
    if mode == 'character':
        ...
        label = temp
        # label = [character_to_radicallist[i[0]].split(' ').append('$') for i in label]
        # print('部首的label为',label)
...
def rectify(mode, string):
    if mode == 'character':
        ...
        # 利用编辑距离找最像的
        if string not in radicals:
            string = find_most_likely(string, radicals)
        return string
    elif mode == 'strokelet':
        # 利用编辑距离找最像的
        if string not in strokelets:
            string = find_most_likely(string, strokelets)
        return string

# 要控制这个函数不要被执行多次，因为很影响效率
# 用一个字典实现某个epoch的单例
# 这里有bug
confusing_feature_strokelet = None
def get_support_sample_feature_strokelet(model, root):
    model.eval()
    # 这里可以优化一下，只挑会混淆的样本，创建一个比较小的字符表
    # 一共就340个样本
    f = open(root, 'rb')
    ...
    with torch.no_grad():
        ...
        for iteration in range(train_loader_len):
            ...
            # length, text_input, text_gt, _ = converter('strokelet', label)
            conv_features = model(image, None, None)['conv']

            # conv_features = torch.mean(conv_features,1).unsqueeze(1)
            # print(conv_features)
            gallery.append(conv_features)

        gallery_combine = torch.cat([i for i in gallery], 0)
    # 会返回一个张量 340, 1024, 16, 16
    return confusing_dict, gallery_combine

# 判断是否预测正确的函数
# is_correct都要转化为字符级的才对 / gt设置为strokelet / word_label就是对应的字
# 要保证pred和gt都不会有 '$' 结束符号
def is_correct(epoch, model, mode, probe_feature, pred, gt, word_label, clean_cache):
    ...
    if mode == 'character':
        ...
        # 判断一下是否是混淆radical
        if word_label != '士' and word_label != '土':
            return {
                'correct': True
            }
        else:
            ...
            if confusing_feature_strokelet is None:
                # print('None!重新计算特征值')
                confusing_dict1, gallery_combine1 = get_support_sample_feature_strokelet(model, './data/10_confusing_340_simsun.pkl')
                confusing_dict2, gallery_combine2 = get_support_sample_feature_strokelet(model, './data/10_confusing_340_simkai.pkl')
                confusing_dict3, gallery_combine3 = get_support_sample_feature_strokelet(model, './data/10_confusing_340_simfang.pkl')

                confusing_feature_strokelet = [] # 当前epoch的单例模式
                ...
                confusing_feature_strokelet.append((confusing_dict3, gallery_combine3))
            else:
                # print('不为None!')

                confusing_dict1, gallery_combine1 = confusing_feature_strokelet[0]
                confusing_dict2, gallery_combine2 = confusing_feature_strokelet[1]
                confusing_dict3, gallery_combine3 = confusing_feature_strokelet[2]
            # 通过判别出来的笔顺序列获取一个混淆列表
            confusing_list = confusing_dict1['121'] # 工、 士、 土
            # 混淆列表的特征
            ...
            gallery_feature3 = gallery_combine3[torch.Tensor(confusing_list).long()]

            scores1 = torch.Tensor([mse_loss(probe_feature, i) for i in gallery_feature1[1:]]).view(-1,1)
            scores2 = torch.Tensor([mse_loss(probe_feature, i) for i in gallery_feature2[1:]]).view(-1,1)
            scores3 = torch.Tensor([mse_loss(probe_feature, i) for i in gallery_feature3[1:]]).view(-1,1)
            scores = torch.mean(torch.cat([scores1, scores2, scores3],1),1)




            if scores[0] <= scores[1] and word_label == '士':
                return {
                    'correct': True
                }
            if scores[0] > scores[1] and word_label == '土':
                return {
                    'correct': True
                }
            return {
        ...
        # 适用于韩文文本
        if True:
        # if config['alphabet'] == 'korean' or config['alphabet'] == 'korean+3755' or config['alphabet'] == 'ctw':
            if pred == gt:
                return {
                    'correct': True
                }
        ...
        # 判断一下是否是混淆strokelet
        # if False:
        if len(confusing_strokelet_dic[pred]) == 1:
            return {
                'correct': True
            }
        else:
            ...
            # 如果预测的序列不在dict中，直接返回false，340个字已经涵盖了CTW和LEVEL1的混淆集
            if pred not in confusing_dict1:
                print('errrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrror')
                return {
                    'correct': False
                }
            ...
            scores3 = torch.Tensor([mse_loss(probe_feature, i) for i in gallery_feature3]).view(-1,1)

            # scores1 = torch.Tensor([1-torch.cosine_similarity(probe_feature.view(-1), i.view(-1), dim=0) for i in gallery_feature1]).view(-1,1)
            # scores2 = torch.Tensor([1-torch.cosine_similarity(probe_feature.view(-1), i.view(-1), dim=0) for i in gallery_feature2]).view(-1,1)
            # scores3 = torch.Tensor([1-torch.cosine_similarity(probe_feature.view(-1), i.view(-1), dim=0) for i in gallery_feature3]).view(-1,1)
            ...
            # 代码好像有bug
            # confusing_character = confusing_character_340[min_index]
